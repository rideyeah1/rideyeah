// World Cup 2026 posts as 9:16 stories (1080x1920) for IG/FB stories.
...
<div class="stage">
  <div style="position:absolute;inset:0;background:url('${cfg.img}') ${cfg.bgx} center / cover no-repeat"></div>
  <div style="position:absolute;left:0;right:0;top:0;height:26%;background:linear-gradient(180deg,rgba(8,8,9,.82),rgba(8,8,9,.28) 50%,transparent)"></div>
  <div style="position:absolute;left:0;right:0;bottom:0;height:58%;background:linear-gradient(0deg,rgba(8,8,9,.96) 0%,rgba(8,8,9,.9) 22%,rgba(8,8,9,.5) 52%,transparent)"></div>
  <div style="position:absolute;inset:24px;border:2px solid rgba(201,168,106,.32);border-radius:14px;pointer-events:none"></div>

  <!-- top: centered logo (kept clear of story top UI) -->
  <div style="position:absolute;top:128px;left:0;right:0;display:flex;align-items:center;justify-content:center;gap:20px">
    ${badge(82)}${wordmark(46)}
  </div>

  <!-- bottom content block, lifted ~300px to clear story reply bar -->
  <div style="position:absolute;left:74px;right:74px;bottom:300px">
    <div class="eyebrow" style="font-size:28px;letter-spacing:.30em;margin-bottom:20px">${cfg.eyebrow}</div>
    <div class="serif" style="font-size:${cfg.h1Px}px;line-height:1.0;font-weight:600;letter-spacing:-.015em;color:${BONE};text-shadow:0 3px 22px rgba(0,0,0,.6)">${cfg.h1}</div>
    <div style="height:2px;width:130px;background:linear-gradient(90deg,${GOLD},rgba(201,168,106,0));margin:30px 0 24px"></div>
    <div style="font-size:33px;line-height:1.34;color:${BONE};font-weight:500;text-shadow:0 2px 14px rgba(0,0,0,.78)">${cfg.sub}</div>
    <div style="margin-top:40px;display:flex;flex-direction:column;gap:22px">
      <span style="align-self:flex-start;display:inline-flex;align-items:center;background:${GOLD};color:#15110a;font-weight:800;font-size:33px;letter-spacing:.01em;padding:20px 40px;border-radius:999px;box-shadow:0 8px 26px rgba(0,0,0,.45)">${cfg.cta}</span>
      <div style="font-size:31px;font-weight:700;color:${BONE};text-shadow:0 1px 10px rgba(0,0,0,.8)">${cfg.site}<span style="color:rgba(201,168,106,.7);margin:0 14px">&#9670;</span>${cfg.phone}</div>
    </div>
  </div>
</div></body></html>`;
...
const JOBS = [
  {
    name: 'story-worldcup-stadium', img: IMG1, bgx: '52%', h1Px: 86,
    eyebrow: 'Match day &middot; Los Angeles 2026',
    h1: 'Get to every<br>game in style.',
    sub: 'Skip the traffic and parking &mdash; your private black SUV drops you right at the gate.',
    cta: 'Reserve your ride', phone: '(805) 285-1570', site: 'rideyeah.com',
  },
  {
    name: 'story-worldcup-arrival', img: IMG2, bgx: '40%', h1Px: 86,
    eyebrow: 'Summer 2026 &middot; Los Angeles',
    h1: 'Travel like<br>the home team.',
    sub: 'Door-to-door black-SUV service for fans, families &amp; VIPs &mdash; LAX, hotels and stadiums.',
    cta: 'Reserve your ride', phone: '(805) 285-1570', site: 'rideyeah.com',
  },
];
...
for (const j of JOBS) {
  ...
  const outJpg = `C:/Users/reyna/Downloads/RideYeah_${j.name.replace('story-worldcup-', 'WorldCup_Story_')}.jpg`;
  await sharp(pngPath).resize(W, H).png().toFile(outPng);
  await sharp(pngPath).resize(W, H).jpeg({ quality: 92 }).toFile(outJpg);
